...
def can_read_permission(func):
    ...
    def decorated_view(*args, **kwargs):
        if not current_app.config.get('WIKI_READ_PERMISSION')():
            abort(403)
...
def can_edit_permission(func):
    ...
    def decorated_view(*args, **kwargs):
        print('debug')
        if not current_app.config.get('WIKI_EDIT_PERMISSION')():
            abort(403)
        return func(*args, **kwargs)
    return decorated_view
...
def permission_processor():
    return dict(
        can_edit_wiki=current_app.config.get('WIKI_EDIT_PERMISSION')(),
        can_read_wiki=current_app.config.get('WIKI_READ_PERMISSION')()
    )
...
@can_read_permission
def index():
    return page(current_app.config.get('WIKI_HOME'))
...
@can_read_permission
def files():
    if request.method == 'POST' and current_app.config['WIKI_EDIT_PERMISSION']():
    ...
    if request.method == 'POST' and not current_app.config['WIKI_EDIT_PERMISSION']():
